    ...
    public partial class ManageUser : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!IsPostBack)
            {
                ...
                con2.Open();
                var sqlQueryRecentUser = "SELECT UserID FROM dbo.Person WHERE Username=@Username";
                var com1 = new SqlCommand(sqlQueryRecentUser, con2);
                com1.Parameters.AddWithValue("@Username", Session["Username"]);
                var srd = com1.ExecuteReader();
                var hasReadValue = srd.Read();
                var hasUser = false;
                if (
                    hasReadValue) //checks if database is empty, if it is not, it aquires the UserID for future use, if not, then it tells the user that no user exists

                    userID = int.Parse(srd.GetValue(0).ToString());
                ...
                srd.Close();
                if (
                    !hasUser) //if their is a user, uses the aquired ID to place the correct info into text boxes                    
                {
                    var sqlQuery =
                        "SELECT Username, Email, FirstName, LastName, Organization, Reason FROM Person WHERE UserID = @UserID";
                    var comm = new SqlCommand(sqlQuery, con2);
                    var param = new SqlParameter("@UserID", Session["UserID"]);
                    ...
                    while (srd.Read())
                    {
                        txtusername.Text =
                            srd.GetValue(0)
                                .ToString(); //gets value from the corresponding column in the query                            
                        txtoldpassword.Text = "";
                        txtnewpassword.Text = "";
                        txtconpassword.Text = "";
        ...
        protected void btnconfirm_Click(object sender, EventArgs e)
        {
            ...
            if (hasUser && txtnewpassword.Text.Equals(txtconpassword.Text) &&
                (txtnewpassword.Text != null || txtconpassword.Text != null) &&
                (!txtnewpassword.Text.Equals("") || !txtconpassword.Text.Equals("")))
            {
                var sqlQuery = "UPDATE dbo.Person SET Username=@Username, Email=@Email" +
                ...
                var passUpdate = new SqlCommand();
                passUpdate.Connection = con2;
                passUpdate.CommandType = CommandType.StoredProcedure;
            ...
            else if (hasUser && (txtoldpassword.Text == null || txtoldpassword.Text.Equals("")) &&
                     (txtnewpassword.Text == null || txtnewpassword.Text.Equals("")) &&
                     (txtconpassword.Text == null ||
                      txtconpassword.Text.Equals(""))) //changes newest entry into database if a user is in database
